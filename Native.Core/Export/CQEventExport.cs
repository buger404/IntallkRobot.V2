...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,io.github.buger404.intallk";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("io.github.buger404.intallk", 1, 9, "黑嘴II", "1.0.0", 1, "Buger404", "黑嘴机器人", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("io.github.buger404.intallk", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("io.github.buger404.intallk", AppData.CQLog);	
